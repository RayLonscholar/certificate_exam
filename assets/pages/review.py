...
class Review():
    def __init__(self, page):
        ...
        def start_clicked(e):
            ...
            def create_topic_information(content):
                ...
                self.topic_information.controls.append(
                    Text("{}.({})：{}".format(self.index, content[2], order[0][4:]), size = 20, weight = "bold")
                )
                # 題目圖片
                if content[1] != None:
                    self.topic_img_edit.value = f"{content[1]}"
                    self.topic_information.controls.append(
                        Image(src=f"{content[1]}")
                    )
                for _ in order[1:]:
                    self.topic_information.controls.append(
                ...
                def topic_edit_submit_clicked(e):
                    ...
                    self.topic.visible = False # 隱藏第二區塊
                    self.topic_information_main.visible = False # 隱藏第三區塊
                    self.edit_content.visible = False # 顯示修改內容選項
                    self.topic_edit_main.visible = False # 隱藏第四區塊
                    write_list()
                    page.update()
        ...
        self.topic = Column(
            ...
            visible = False,
            controls = [
                Text("請選擇題目", size = 20, weight = "bold"),
                Column(
                    expand = True,
                    scroll = "ADAPTIVE",
